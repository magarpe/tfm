...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      pattern = '<%sH'%length
      buff.write(struct.pack(pattern, *self.mObservations_KFIDs))
      ...
      buff.write(_struct_I.pack(length))
      pattern = '<%sH'%length
      buff.write(struct.pack(pattern, *self.mObservations_n))
      buff.write(_get_struct_3f().pack(*self.mNormalVector))
      buff.write(_get_struct_B().pack(self.mbNormalAndDepthChanged))
      _x = self.mDescriptor
      # - if encoded as a list instead, serialize as bytes instead of string
      if type(_x) in [list, tuple]:
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.mObservations_KFIDs = struct.unpack(pattern, str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      start = end
      ...
      self.mObservations_n = struct.unpack(pattern, str[start:end])
      start = end
      end += 12
