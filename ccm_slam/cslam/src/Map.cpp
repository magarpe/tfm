...
{
    {
        ...
        if(mSysState == SERVER)
        {
            if(mmpKeyFrames.size() % 50 == 0)
            {
                ...
                //MSGP add to print kfs without mapfusion or loopclosure
...
void Map::WriteStateToCsv(const std::string& filename,
  ...
  for (std::map<idpair,kfptr>::const_iterator itr = mmpKeyFrames.begin();       //mmpKeyFrames son las poses (KF)
...
//    const cv::Mat T_wc = pKF->GetPoseInverse();
    const cv::Mat T_wc = pKF->GetPose();
    const Eigen::Matrix4d eT_wc = Converter::toMatrix4d(T_wc);

    ccptr pCC = *(mspCC.begin());
    Eigen::Matrix4d T_SC;
    if(mSysState == SERVER)
        T_SC = pKF->mT_SC;      //mT_SC: Transformation to body frame (for KF write-out)
    else
        T_SC = pCC->mT_SC;

    const Eigen::Matrix4d Tws = eT_wc * T_SC.inverse();       //Tws=[R  x]
                                                              //        y]
                                                              //        z]
                                                              //  0 0 0 1]
    const Eigen::Quaterniond q(Tws.block<3,3>(0,0));        //quaternion from rotation


//    cout << "T_wc = "<< T_wc << endl;
//    cout << "eT_wc = "<< eT_wc << endl;
//    cout << "T_SC = "<< T_SC << endl;
//    cout << "T_SC.inverse() = "<< T_SC.inverse() << endl;

//    keyframesFile << std::setprecision(25) << stamp << ",";                           // time stamp (mTimeStamp)
//    keyframesFile << eT_wc(0,3) << "," << eT_wc(1,3) << "," << eT_wc(2,3);
//    keyframesFile << std::endl;


    keyframesFile << std::setprecision(25) << stamp << ",";                           // time stamp (mTimeStamp)
    keyframesFile << Tws(0,3) << "," << Tws(1,3) << "," << Tws(2,3);           // coordenadas
    keyframesFile << std::endl;

//    keyframesFile << q.w() << "," << q.x() << "," << q.y() << "," << q.z() << ",";  // quaternion: orientation and rotation
//    keyframesFile << vel[0] << "," << vel[1] << "," << vel[2] << ",";               // siempre 0
